...
int Expression_tree::compute_value()
{
	...
	if (result == -2147483648) {
		throw Math_error("Int limit overflow\n");
	}
	return result;
}
...
void Expression_tree::print(System::Windows::Forms::TextBox^ textBox, Expression_tree_node* root, int level)
{
	if (!root)
		return;
	else {
		print(textBox, root->right, ++level);
		print(textBox, root->right_num, ++level);
		for (int i = 0; i < level; i++)
			textBox->Text += "     ";
		if (root->is_leaf) {
			string number = to_string(root->value);
			number += "\r\n";
			textBox->Text += gcnew System::String(number.c_str());
		}
		else {
			string number = root->sign;
			number += "\r\n";
			textBox->Text += gcnew System::String(number.c_str());
		}
		level--;
	}
	print(textBox, root->left, ++level);
	level--;
	print(textBox, root->left_num, ++level);
}
